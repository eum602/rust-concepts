...
fn main() {
    ...
    fn add(a: &i32, b: &i32) -> i32 {
        let a_dereferenced = *a;
        let b_dereferenced = *b;
        a_dereferenced + b_dereferenced //the same result can be acomlished bia references
    }
...
// 2. Write a function `change` that takes a *mutable* reference to a String and adds an "s" to
    ...
    //
fn change(s: &mut String) { //passing a mutable reference to a string, so it can modify the value
    if !s.ends_with("s") {
        s.push_str("s");
    }
}
...
//
fn eat(s: String) -> bool { //passing the value directly makes
    s.starts_with("b") && s.contains("a") //elegant to return directly the bool
}