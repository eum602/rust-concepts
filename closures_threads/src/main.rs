...
fn expensive_sum(v: Vec<i32>) -> i32 {
    ...
    v.iter()
        // .filter() goes here
        //.filter(|x| *x % 2 == 0) //the param is a reference so deferencing with *x
        .filter(|&x| x % 2 == 0) //here receiving the parameter deferenced by using &x
        // .map() goes here
        .map(|&x| x*x)
        .sum()
}
...
fn main() {
    ...
    //
    let handle = thread::spawn(move || expensive_sum(my_vector)); //move -> explictly make child thread take ownership of the vector
    ...
    let (tx, rx) = channel::unbounded(); //unbounded orders to buffers as much values until the ram runs out of memory
    // Cloning a channel makes another variable connected to that end of the channel so that you can
    // send it to another thread.
    let (tx2, rx2) = (tx.clone(), rx.clone());
//##############################################################//
    // let (tx3,rx3) = (tx2.clone(), rx2.clone());
    // let (tx4,rx4) = (tx3.clone(), rx3.clone());
//##############################################################//
...
//##########################NOT WORKING####################################//
    ...
    //     println!("Received from thread 3 ==> {}",received);
    // });

    // let handle_d = thread::spawn(move || {
    //     let received = rx4.recv().unwrap();
    //     println!("Received from thread 4 ==> {}",received);
    // });

    // for element in vec!["1","2","3","4"] {
    //     tx3.send(element).unwrap();
    //     tx4.send(element).unwrap();
    // }
