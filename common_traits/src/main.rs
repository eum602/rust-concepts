#[derive(Debug, Clone ,Copy, PartialEq)] //adding this otherwise ->  `Cake` cannot be formatted using `{:?}` when using a println
                              // Copy is a subrait of clone thus we need to also implement Clone to use Copy
pub enum Cake {
    Chocolate,
    MapleBacon,
    Spice,
...
impl Default for Party {
    fn default() -> Self {  // 'Self' -> instead of putting 'Party'
           Self {
            at_restaurant: true,
            num_people: 8,
            cake: Cake::Chocolate,
...
fn main() {
    ...
    let cake = Cake::Spice;
    admire_cake(cake); //thanks to copy here it is put a Copy instead of moving the object
